...
async function createAccount() {
  msg.value = ''
  if (!nName.value.trim() || !nEmail.value.trim() || !nPass.value) {
    msg.value = 'Prénom, email et mot de passe requis.'
    return
  }
  ...
  const params = new URLSearchParams({
    ...
    lien: nLien.value,
    is_admin: String(nAdmin.value),
  })

  const response = await fetch(`${API}/api/members?${params}`, {
    method: 'POST',
    headers: authHeaders(),
  })
  ...
  if (!response.ok) {
    ...
    msg.value = detail.detail || 'Erreur.'
    return
  }
...
async function addLien() {
  ...
  const params = new URLSearchParams({ label: newLien.value.trim() })
  const response = await fetch(`${API}/api/liens?${params}`, {
    method: 'POST',
    headers: authHeaders(),
  })
  ...
  nLien.value = newLien.value.trim()
  newLien.value = ''
}

async function removeMember(member) {
  if (!confirm(`Supprimer le compte de ${member.name} et ses tâches ?`)) return
  ...
  if (!response.ok) {
    const detail = await response.json().catch(() => ({}))
    alert(detail.detail || 'Suppression impossible.')
    return
  }
...
async function toggle(task) {
  await fetch(`${API}/api/tasks/${task.id}`, { method: 'PATCH', headers: authHeaders() })
  await refresh()
}
...
<template>
  <div class="view">
    ...
    <div class="card">
      ...
      <div class="row"><input v-model="nEmail" type="email" placeholder="Email" /></div>
      <div class="row"><input v-model="nPass" type="password" placeholder="Mot de passe" /></div>
      <div class="row">
        <select v-model="nLien">
      ...
      <div class="row addlien">
        <input v-model="newLien" placeholder="Ajouter un lien" @keyup.enter="addLien" />
    ...
    <div class="card">
      ...
      <div class="mlist">
        <div v-for="member in members" :key="member.id" class="mrow">
          <span class="avatar sm">{{ initials(member.name) }}</span>
          <span class="mname">{{ member.name }}</span>
          <span class="who">{{ member.lien }}</span>
          <span class="badge" v-if="member.is_admin">admin</span>
          <button v-if="member.id !== me.id" class="trash" @click="removeMember(member)" title="Supprimer">🗑</button>
        </div>
      </div>
    </div>
